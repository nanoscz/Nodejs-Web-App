...
userRoutes.route('/signup')
    .get(function(req, res,next) {
        if(req.session.user){
            ...
            res.render("signup",{title:"Направи Профил", description:"Креирајте свој профил за да може да ги користите сите функционалности.", message:""});
        }
    })
    .post(function(req, res) {
        var form = new formidable.IncomingForm();
        form.parse(req, function (err, fields, files) {
            ...
            var callback = function(err, username) {
                ...
                if (username){
                    res.render("signup",{title:"SignUp", description:"Sign up Page description", message:"Веќе постои тоа корисничко име."});
                }else{
